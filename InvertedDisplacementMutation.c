...
void performInvertedDisplacementMutation(Genetic * genetic) {
    int i;
    for (i = 0; i < genetic->chromosomeNumber; i++) {
        ...
        int rnd3 = rand() % genetic->cityNumber;

        if (rnd1 == 0 && rnd2 >= genetic->cityNumber - 1) {
            rnd1++;
            rnd2--;
        }

        while (rnd3 >= rnd1 && rnd2 >= rnd3) {
            rnd3 = rand() % genetic->cityNumber;
        }
